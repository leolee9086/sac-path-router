...
export class Router {
    ...
    constructor(options = {}) {
        ...
        this.matcher = createMatcher(options.matcher ?? 'regexp');
    }
    /**
    ...
    register(path, methods, middleware, opts = {}) {
        ...
        for (const [name, handler] of Object.entries(this.params))
            layer.param(name, handler);
        this.stack.push(layer);
        this.matcher.add(layer);
        if (layer.name !== null)
            this.named.set(layer.name, layer);
        return layer;
    }
    /**
        ...
        for (const entry of rest) {
            if (entry instanceof Router) {
                for (const layer of entry.stack) {
                    const mounted = new Layer(`${path ?? ''}${layer.path}`, layer.methods, [...layer.stack], {
                        ...layer.opts,
                        name: layer.name,
                        prefix: '',
                    });
                    ...
                    if (mounted.name !== null)
                        this.named.set(mounted.name, mounted);
                }
                continue;
            }
            if (typeof entry !== 'function')
                throw new TypeError('use() expects middleware, a router, or a path');
            this.register(path ?? '([^/]*)', [], entry, {
                end: false,
    ...
    verb(method, args) {
        ...
        for (const entry of rest) {
            if (typeof entry !== 'function')
                throw new TypeError(`${method} ${path}: middleware must be a function`);
        }
        this.register(path, method === 'all' ? [] : [method], rest, { name });
        return this;
    }
    /** Route `GET` (and `HEAD`). */
    get(...args) { return this.verb('get', args); }
    /** Route `POST`. */
    post(...args) { return this.verb('post', args); }
    /** Route `PUT`. */
    put(...args) { return this.verb('put', args); }
    /** Route `PATCH`. */
    patch(...args) { return this.verb('patch', args); }
    /** Route `DELETE`. */
    delete(...args) { return this.verb('delete', args); }
    /** Route `HEAD`. */
    head(...args) { return this.verb('head', args); }
    /** Route `OPTIONS`. */
    options(...args) { return this.verb('options', args); }
    /** Route every method. */
    all(...args) { return this.verb('all', args); }
    ...
    group(path, configure) {
        const child = new Router(this.settings);
        configure(child);
    ...
    url(name, ...args) {
        ...
        if (layer === undefined)
            throw new Error(`sac-path-router: no route named "${name}"`);
    ...
    routes() {
        ...
        return function dispatch(context, next) {
            ...
            const mostSpecific = entries[entries.length - 1].layer;
            context.routePath = mostSpecific.path;
            context.routeName = mostSpecific.name;
            const chain = [];
            for (const entry of router.settings.exclusive ? [entries[entries.length - 1]] : entries) {
    ...
    allowedMethods(options = {}) {
        ...
        return async (context, next) => {
            ...
            for (const layer of context.matched)
                for (const method of layer.methods)
                    allowed.add(method);
            const list = [...allowed].join(', ');
            if (!implemented.includes(context.method.toLowerCase())) {
                if (options.throw === true)
                    throw new Error('sac-path-router: method not implemented');
                context.status = 501;
            ...
            if (!allowed.has(context.method)) {
                if (options.throw === true)
                    throw new Error('sac-path-router: method not allowed');
                context.status = 405;
                context.responseHeaders.set('Allow', list);
            }
        };
